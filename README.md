...
> **当前分支 `cleaner` 是清理器（SafeCleaner）。**
> `main` 分支上的 iOSCleanerInspector 仍是**只读审计工具**，保持不变 —— 它就是用来验证这个清理器的。
> 下面的正文描述的是 `main` 上的审计工具；清理器见本节末尾。

一个**只读**的 iOS / TrollStore 文件系统审计工具。0.3.0 起目标收窄为一件事：

> **审计被审计的清理工具「会删除」的那些目录里，到底装了什么。**

不再和它比数字，也不再枚举不该删的目录。

> **安全约束（重要）**
> 本项目从头到尾不含任何删除、移动、重命名、写入被扫描路径的代码。
> 它的目的是把"要被删掉的东西"摊开显示出来，而不是像清理工具那样把一切都压缩成一个"可回收 1.2 GB"的数字。
...
| --- | ---: | ---: | --- |
| `/tmp` | ✅ | ✅ | 系统临时文件，可作为清理目标 |
| `/var/tmp` | ✅ | ✅ | 系统临时文件，可作为清理目标（与 `/tmp` 是同一目录） |
| `/var/mobile/Library/Caches` | ✅ | ✅ | 系统级缓存，可作为主要清理目标 |
| `*/Containers/.../tmp` | ✅ | ✅ | 各 App/Extension sandbox 临时目录，可作为清理目标，**但应跳过正在使用的文件** |
...
| `/var/containers/Data` | ❌ | ❌ | 系统容器数据上层目录，禁止删除 |

评审意见：**不能删除的，我们连扫都不扫。** 所以 0.3.0 把这些路径从扫描列表里删掉了，
并且顺带**把对应的 entitlements 一并收窄** —— 既然不读，就不该持有读权限。
同一原则下，评审表里没列但性质相同的两个也一并停扫：
`Containers/Data/PluginKitPlugin`（插件容器）与 `Containers/Shared/AppGroup`（共享容器）。

注意：**被审计的 App 界面上是有「照片缓存」清理按钮的**（`cleanPhotosCache` 摆在那儿）。
也就是说它提供了一项评审认为"禁止删除"的操作 —— 这本身就是审计结论之一。

## 0.3.0 的审计输出

对每个"会删除"的目标，报告的内容不再是"总共多大"，而是：

- **体积/文件数/目录数**
- **修改时间分布**：`<1h / <24h / <7d / <30d / 更早` 各占多少字节
- **内容构成**：状态类（sqlite / realm / db / plist / wal）、媒体类（mp4/jpg/heic…）、其余
- **风险标记**（每条都是量出来的事实，不是判断）：
  - 含状态类文件 → 应用状态/草稿可能放在 Caches 里，删了无法恢复
  - 最近 1 小时内有写入 → 可能正在使用，清理工具应当跳过（对应评审的"应跳过正在使用的文件"）
  - 含媒体文件 → 可能是用户主动离线保存的内容，不是缓存
- **最大的 15 个文件**：带完整路径、大小、最后修改时间，可以逐个手工核对

报告末尾固定打印一份 **NOT SCANNED** 清单，说明这一版拒绝枚举哪些路径、为什么。
...
## 0.2.x 之前的旧扫描范围（历史记录，0.3.0 已按评审收窄）

命中"可读"的前提下，0.2.x 曾经探测：
...
审计是手段，目标是做一个**安全的清理工具**。规则全部来自 0.3.0 那台真机的实测数据，不是拍脑袋。

**Bundle ID `com.a23bc.SafeCleaner`，与审计工具分开安装，互不干扰。**

## 交互

1. 打开即枚举目标：全局 `/tmp`、`/var/mobile/Library/Caches`，外加每个 App 容器一个条目（显示 bundle id 和体积），**默认全选**，用开关关掉不想动的
2. 每个条目下面是它要清的目录（App 就是 `Library/Caches` 和 `tmp`），**点目录行直接用 Filza 打开自己看**
   （`filza://view/<路径>`，scheme 出自 Filza 官方用户指南）
3. **Dry run** 先出清单：待删文件数、体积、最大的 20 个、以及跳过了多少（见下）
4. 确认后才弹二次确认 → 真删 → 出结果报告（删了多少 / 跳过多少 / 失败多少，失败路径可查）
5. 报告可以导出

## 安全规则（写在 `CleanupPlanner` 里，且有兜底）

- **路径允许清单**（唯一能删的形状）：`/tmp/*`、`/var/mobile/Library/Caches/*`、
  `/var/mobile/Containers/Data/Application/<UUID>/{Library/Caches,tmp}/*`。UUID 要过正则。
  评审判定的禁删目录**根本不在这个清单里**，所以没有任何代码路径能碰到它们
- **只删普通文件**：目录不删（删 `Caches` 目录本身会让 App 出错），符号链接不删也不跟随
- **跳过最近 1 小时内被写入的**（对应评审"应跳过正在使用的文件"）
- **删除前逐条重新 stat 复核**：文件消失/类型变了/又变成 1 小时内写入 → 跳过
- **entitlements 只给这些路径 read-write**：禁删目录连写权限都没申请 —— 就算代码写错，
  沙盒也会先拦住（纵深防御）
- 不用"跳过所有 sqlite/plist"这种规则：0.3.0 的数据证明那些大多就是缓存本身
  （CloudKit ChunkStoreDatabase、ImageTables、地图瓦片），一刀切只会误伤挡不住风险

## 0.1.0 已知限制

- 枚举和 dry run 各要几分钟（跟审计工具一个量级），界面上有转圈提示
- 「只删 30 天以上」这条**还没做开关**，目前规则是"跳过 1 小时内写入"
  —— 要加年龄阈值是一行改动，等你试完再定
- 删除没有撤销，请先用 dry run + Filza 看清楚再动手

---

## 变更记录

### 0.3.0（方向调整：从"比对数字"改为"删除目录的安全审计"）

- **不再与 iOSCleanerPro 比数字**，分类小计整段移除
- **停扫所有评审判定不可删除的路径**：Logs、Downloads、PhotoData/*、InternalDaemon、
  `/var/containers/Data`，以及性质相同的 PluginKitPlugin、Shared/AppGroup
- **entitlements 同步收窄**：去掉这些路径的绝对路径例外，并去掉不再使用的
  `AppBundles`、`MobileDocuments`
- 新增 `Auditor`：对每个删除目标给出修改时间分布、内容构成（状态类/媒体类）、
  风险标记、以及最大的若干文件（含完整路径）
- 新增「最近 1 小时内被写入」统计，对应评审要求的"应跳过正在使用的文件"
- 报告末尾固定打印 NOT SCANNED 清单
- minor 版本号 +1：这是范围与用途的断点式改动，不是小修补
